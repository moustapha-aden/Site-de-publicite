...
const Dashboard = ({ userData, handleLogout }) => {
  ...
  useEffect(() => {
    // 1. Définir le header d'autorisation global (déjà fait par l'intercepteur)
    // 2. Charger les données du tableau de bord
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Simuler la fusion des données utilisateur (reçues du login) et des stats (reçues du dashboard)
        setDashboardData({
          user: userData.user,
    ...
    fetchDashboardData();
  }, [userData.user.name, handleLogout]); // Dépend de handleLogout et du nom utilisateur
